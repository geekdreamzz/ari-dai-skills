...
@app.command()
def sync(
    force: bool = typer.Option(False, "--force", "-f", help="Bypass cache and re-fetch schema from API"),
    skills_dir: Optional[str] = typer.Option(None, "--skills-dir", help="Output dir (default: skills/ relative to this repo)"),
):
    """Fetch platform schema, refresh tool cache, and regenerate SKILL.md files."""
    if not _state.is_authenticated():
        console.print("[red]Not authenticated.[/red] Run: dai login --key dsk_xxx")
        raise typer.Exit(1)
    ...
    if force:
        _state.tool_schema_clear()
        console.print("[dim]Cache cleared — fetching fresh schema...[/dim]")

    cached = _state.tool_schema_get_all()
    if cached and not force:
        tools = cached
        console.print(f"  [dim]Using cached schema ({len(tools)} tools — run with --force to refresh)[/dim]")
    else:
        console.print("  Fetching /api/mcp/schema...")
        try:
            client = DaiClient.from_state()
            result = client.get("/api/mcp/schema")
            tools = result.get("tools", [])
            for t in tools:
                if t.get("id"):
                    _state.tool_schema_set(t["id"], t)
            console.print(f"  [green]✓[/green] {len(tools)} tools fetched and cached (24h TTL)")
        except Exception as e:
            console.print(f"  [red]Failed to fetch schema:[/red] {e}")
            raise typer.Exit(1)
    ...
    for t in tools:
        resource = t.get("resource", "misc")
        groups[resource].append(t)

    # ── 3. Generate SKILL.md files ───────────────────────────────────────────
    repo_root = Path(__file__).parent.parent.parent
    out_dir = Path(skills_dir) if skills_dir else repo_root / "skills"
    out_dir.mkdir(exist_ok=True)

    generated = 0
    skipped = 0
    for resource, resource_tools in sorted(groups.items()):
        skill_dir = out_dir / resource
        skill_dir.mkdir(exist_ok=True)
        skill_file = skill_dir / "SKILL.md"

        # Don't overwrite hand-crafted files (those without our auto-gen marker)
        if skill_file.exists():
            content = skill_file.read_text(encoding="utf-8")
            if "<!-- dai-sync: skip -->" in content:
                skipped += 1
                continue
        ...
        lines = [
            f"<!-- auto-generated by dai sync · {_time.strftime('%Y-%m-%d')} · do not edit by hand -->",
            "---",
            f"name: {resource}",
            f"description: {label} tools for Dataspheres AI",
            "---",
            "",
            f"# {label}",
            "",
        ...
        for t in sorted(resource_tools, key=lambda x: x.get("id", "")):
            tool_id = t["id"]
            tool_label = t.get("label", tool_id)
            desc = t.get("skillDescription", "").strip()
            fields = t.get("fields", [])

            lines.append(f"### `{tool_id}` — {tool_label}")
            lines.append("")
            if desc:
                lines.append(desc)
                lines.append("")
            if fields:
                ...
                for f in fields:
                    ...
                    fdesc = f.get("description", "").replace("|", "\\|")
                    lines.append(f"| `{f['name']}` | {f.get('type','string')} | {req} | {fdesc} |")
                lines.append("")

        skill_file.write_text("\n".join(lines) + "\n", encoding="utf-8")
        generated += 1

    console.print(f"  [green]✓[/green] Generated {generated} SKILL.md file(s) in {out_dir}")
    if skipped:
        console.print(f"  [dim]Skipped {skipped} hand-crafted file(s) (<!-- dai-sync: skip --> marker)[/dim]")
    console.print("\n[dim]Add <!-- dai-sync: skip --> to any SKILL.md to protect it from overwrites.[/dim]")
