...
> **The protocol is enforced and templated — see [PROTOCOL.md](PROTOCOL.md).** Every board item
...
**Journey coverage is mandatory.** Every NS must enumerate its USER JOURNEYS in the Success Criteria — one per actor (e.g. *creator*: create → configure → publish → share; *guest*: scan QR → view → upload → see it appear), and the regression wall must contain at least one requirement spec per journey, end to end. A feature can pass every task-level gate and still ship broken if a journey was never written down: the live-gallery publish gap happened because EX tasks scoped components (toggle, QR button) and the guest flow, while the creator's publish step existed in no spec at all. Gates verify what specs contain — only journey enumeration catches what specs omit. During AUDIT/DONE modes, check each NS journey against the regress suite; a journey without a spec is a gap to file as intake, not a footnote.
